...
with tf.name_scope("WeightsBiases"):
...
def neuralnetBackProp(inputData):
	...
	with tf.name_scope("Optimizer"):
		gradOptimizer = tf.train.AdamOptimizer(learning_rate = 0.001).minimize(cost)	# Using different optimizer.
	...
	with tf.Session() as sess:
		...
		writer = tf.summary.FileWriter("tmp/DeepLearning/27")
		writer.add_graph(sess.graph)
		
		for i in range(n_iter):
			_, c = sess.run([gradOptimizer,cost], feed_dict={X: xtrain, Y: ytrain})
			summary = sess.run(merged_summary, feed_dict={X: xtrain, Y: ytrain})
			writer.add_summary(summary, n_iter)

			print("Loss after iteration ",i," : ",c)
